...
export type ThreadDetailRouteContext = {
...
export function createThreadDetailHandlers(
    ...
    async function GET(request: Request, { params }: ThreadDetailRouteContext) {
    ...
    async function PATCH(request: Request, { params }: ThreadDetailRouteContext) {
        const actor = await dependencies.getActor(request);
        if (actor === null) {
            return Response.json(
                { error: "Not authenticated" },
