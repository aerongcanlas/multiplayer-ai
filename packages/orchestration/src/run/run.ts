...
import type { EventSink, RunInput, RunRecord, RunStore } from "./ports";
...
export type RunDeps = Omit<ToolDeps, "runId" | "roomId" | "sink" | "profile"> & {
    sink: EventSink;
    store: RunStore;
...
export async function startRun(input: RunInput, deps: RunDeps): Promise<string> {
    ...
    try {
        ...
        if (profile.verificationPass) {
            ...
            try {
                ...
                if (controller.signal.aborted || !NoOutputGeneratedError.isInstance(error)) {
                    throw error;
                }
